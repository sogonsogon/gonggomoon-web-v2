import { Strategy, StrategyResult } from '@/features/strategy/types';

export const MOCK_STRATEGY_RESULT: StrategyResult = {
  strategyId: '9b2204c5-4282-4e3b-9870-04b14a901e45',
  postAnalysisId: 'e9e77335-672a-408b-ac2a-010a98e4d86a',
  postAnalysisTitle: '토스 Backend 신입 채용',
  selectedExperienceCount: 3,
  createdAt: '2026-07-01T22:26:00.056',
  mainPositioningMessage:
    '작은 기능을 끝까지 책임지고 개선해 본 경험을 바탕으로, 복잡한 백엔드 시스템에서도 안정성과 사용자 경험을 함께 고려하는 신입 Backend 개발자로 포지셔닝하세요.',
  experienceStrategyPoints: [
    {
      experienceType: 'PROJECT',
      experienceTitle: '카카오 데이터 분석 인턴십',
      strategyPoint: '데이터 기반 의사결정과 지표 해석 역량으로 연결할 수 있습니다.',
    },
    {
      experienceType: 'ACTIVITY',
      experienceTitle: 'UX 스터디 그룹 리더',
      strategyPoint: '사용자 관점에서 문제를 정의하고 협업한 사례로 활용하세요.',
    },
    {
      experienceType: 'PROJECT',
      experienceTitle: '아이디어 분석앱 프로젝트',
      strategyPoint: '요구사항 정리, 서비스 구조 설계, 결과 발표 경험을 묶어 설명하세요.',
    },
  ],
  experienceOrdering: [
    {
      order: 1,
      title: '아이디어 분석앱 프로젝트',
      reason: '핵심 기술 역량을 가장 잘 보여주는 경험',
    },
    { order: 2, title: '카카오 데이터 분석 인턴십', reason: '실무 환경에서의 문제 해결 경험' },
    { order: 3, title: 'UX 스터디 그룹 리더', reason: '협업 및 커뮤니케이션 역량 보완' },
  ],
  keywords: ['문제 해결', '데이터 기반 개선', '협업', '사용자 경험', 'API 설계', '운영 관점'],
  strengths: [
    '지표를 해석하고 개선 우선순위를 정하는 능력',
    '모호한 요구사항을 실행 가능한 작업으로 구체화하는 능력',
    '팀과 API/데이터 흐름을 맞추며 구현하는 협업 능력',
  ],
  kpiCheckList: [
    '문제 상황을 담백하게 설명하고 본인의 판단 근거를 분명히 드러내세요.',
    '신입 지원자인 만큼 배운 점보다 다음 프로젝트에서 어떻게 적용했는지를 강조하세요.',
    '과장된 표현보다 실제 행동, 선택, 결과를 중심으로 작성하세요.',
  ],
  improvementGuides: [
    {
      title: '대용량 트래픽 대응',
      description: '성능 개선을 위해 어떤 지표를 보고 판단했는지 구체화하세요.',
    },
    {
      title: '협업 경험',
      description: '기획자/프론트엔드와 API 스펙을 맞춘 경험을 중심으로 설명하세요.',
    },
    {
      title: '문제 해결 흐름',
      description: '결과 중심보다 문제 발견, 원인 분석, 해결 시도, 재발 방지 흐름을 강조하세요.',
    },
  ],
};

export const MOCK_RECENT_STRATEGIES: Strategy[] = [
  {
    strategyId: '9b2204c5-4282-4e3b-9870-04b14a901e45',
    postAnalysisId: '4cdbde72-5ce9-4543-b4cc-6351264b3824',
    postAnalysisTitle: '당근 Frontend 개발자 채용',
    status: 'READY',
    createdAt: '2026. 06. 02',
  },
  {
    strategyId: '75fd54c8-1269-4c7f-9995-dc3929b2bafc',
    postAnalysisId: '003622f3-153b-4da7-b285-aa57e61e97fd',
    postAnalysisTitle: '토스 Backend 개발자 채용',
    status: 'READY',
    createdAt: '2026. 06. 01',
  },
];
